...
# Pizzaria fechada (23:50-08:30): não confere o KDS nem chama o maná (22/08/26).
FECHA_MIN = 23 * 60 + 50
ABRE_MIN  = 8 * 60 + 30
def pizzaria_fechada():
    t = datetime.datetime.now().time()
    m = t.hour * 60 + t.minute
    return m >= FECHA_MIN or m < ABRE_MIN
...
def main():
    ...
    while True:
        try:
            if pizzaria_fechada():
                if not dormindo:
                    dormindo = True; log("Pizzaria fechada (23:50-08:30): robô do KDS dormindo.")
                time.sleep(60); continue
            if dormindo:
                dormindo = False; primeira = True; log("Pizzaria abrindo: robô do KDS acordou.")
