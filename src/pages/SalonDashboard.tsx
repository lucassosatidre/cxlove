...
export default function SalonDashboard() {
  ...
  // Record operator on first access
  useEffect(() => {
  ...
  const loadData = async () => {
    ...
    const allClosings = (closingsData as SalonClosing[]) || [];
    ...
    if (allClosings.length > 0) {
      ...
      const { data: orders } = await supabase
        .from('salon_orders')
        .select('salon_closing_id, is_confirmed')
        .in('salon_closing_id', closingIds);
      if (orders) {
        const stats: Record<string, OrderStats> = {};
        orders.forEach(o => {
          if (!o.salon_closing_id) return;
          if (!stats[o.salon_closing_id]) stats[o.salon_closing_id] = { total: 0, confirmed: 0 };
          stats[o.salon_closing_id].total++;
          if (o.is_confirmed) stats[o.salon_closing_id].confirmed++;
        });
        setOrderStats(stats);
      }
    }
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const { data: orders } = await supabase.from('salon_orders').select('id').in('salon_import_id', importIds);
  ...
  const handleAbrirCaixa = async () => {
    ...
    if (abrirCaixaDate > todayDate) { toast.error('Não é possível abrir caixa para datas futuras.'); return; }
    setAbrirCaixaLoading(true);
    try {
      const dateStr = format(abrirCaixaDate, 'yyyy-MM-dd');
      const { data: existing } = await supabase.from('salon_closings').select('id').eq('closing_date', dateStr).maybeSingle();
      if (existing) { navigate(`/salon/closing/${existing.id}`); }
      else {
        const { data: newClosing, error } = await supabase.from('salon_closings').insert({ closing_date: dateStr, user_id: user.id, status: 'pending' }).select('id').single();
        if (error) throw error;
        navigate(`/salon/closing/${newClosing.id}`);
      }
      setAbrirCaixaOpen(false);
    } catch (err) { toast.error('Erro ao abrir caixa.'); console.error(err); }
    finally { setAbrirCaixaLoading(false); }
  };

  const handleDeleteClosing = async () => {
    if (!deleteClosingDialog) return;
    const expectedDate = formatDate(deleteClosingDialog.date);
    if (deleteConfirmDate !== expectedDate) { toast.error(`Digite a data corretamente: ${expectedDate}`); return; }
    setDeleteClosingLoading(true);
    try {
      const closingId = deleteClosingDialog.id;
      const { data: closingImports } = await supabase.from('salon_imports').select('id').eq('salon_closing_id', closingId);
      const importIds = closingImports?.map(i => i.id) || [];
      const { data: orders } = await supabase.from('salon_orders').select('id').eq('salon_closing_id', closingId);
      const orderIds = orders?.map(o => o.id) || [];
      ...
      if (importIds.length > 0) { await supabase.from('salon_imports').delete().in('id', importIds); }
      await supabase.from('salon_card_transactions').delete().eq('salon_closing_id', closingId);
      ...
      await supabase.from('salon_closings').delete().eq('id', closingId);
      toast.success(`Fechamento do dia ${expectedDate} excluído.`);
      setDeleteClosingDialog(null); setDeleteConfirmDate('');
      await loadData();
    } catch (err) { toast.error('Erro ao excluir fechamento.'); console.error(err); }
    finally { setDeleteClosingLoading(false); }
  };
  ...
  const renderClosingCard = (closing: SalonClosing, isToday: boolean) => {
      ...
      <div key={closing.id} className={isToday ? 'border-2 border-primary/30 rounded-xl bg-primary/5' : ''}>
        ...
        >
          <div className="flex items-center gap-4 flex-1 min-w-0">
            <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center shrink-0", isComplete ? "bg-success/10" : "bg-primary/10")}>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Badge variant={isComplete ? 'default' : 'secondary'} className={isComplete ? 'bg-success text-success-foreground' : 'bg-warning/15 text-warning border-warning/30'}>
                ...
                {closingImports.map((imp) => (
                  <div key={imp.id} className="flex items-center justify-between text-xs bg-muted/50 rounded-lg px-3 py-2">
                    ...
                    <div className="flex items-center gap-3 text-muted-foreground">
                      ...
                      {imp.skipped_cancelled > 0 && <span className="text-destructive">{imp.skipped_cancelled} cancelados</span>}
  ...
  return (
        ...
        isAdmin ? (
          <Popover open={abrirCaixaOpen} onOpenChange={setAbrirCaixaOpen}>
            ...
            <PopoverContent className="w-auto p-0" align="end">
              <div className="p-3 border-b border-border"><p className="text-sm font-medium text-foreground">Selecione a data</p></div>
              <Calendar mode="single" selected={abrirCaixaDate} onSelect={setAbrirCaixaDate} today={undefined} disabled={{ after: new Date() }} initialFocus className={cn("p-3 pointer-events-auto")} />
    ...
    >
      ...
      {selectedImports.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-foreground text-background rounded-xl shadow-lg px-5 py-3 flex items-center gap-4 z-50">
          ...
          <Button size="sm" variant="destructive" onClick={handleDeleteSelected} disabled={deleting}>
            <Trash2 className="h-4 w-4 mr-1.5" />{deleting ? 'Apagando...' : 'Apagar'}
          </Button>
          <Button size="sm" variant="ghost" className="text-background hover:text-background/80 hover:bg-background/10" onClick={() => setSelectedImports(new Set())}>Cancelar</Button>
        </div>
      )}
      ...
      <Dialog open={!!deleteClosingDialog} onOpenChange={(open) => { if (!open) { setDeleteClosingDialog(null); setDeleteConfirmDate(''); } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-destructive"><ShieldAlert className="h-5 w-5" />Excluir Fechamento</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Para confirmar, digite a data: <strong>{deleteClosingDialog ? formatDate(deleteClosingDialog.date) : ''}</strong></p>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteClosing} disabled={deleteClosingLoading || !deleteConfirmDate}>
              <Trash2 className="h-4 w-4 mr-1.5" />{deleteClosingLoading ? 'Excluindo...' : 'Excluir Permanentemente'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
