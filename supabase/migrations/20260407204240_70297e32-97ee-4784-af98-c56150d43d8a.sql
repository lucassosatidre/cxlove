...
-- Partial unique index: only one active checkin per driver per shift
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_active_checkin_per_driver_shift
ON delivery_checkins (driver_id, shift_id)
WHERE status IN ('confirmado', 'fila_espera', 'em_rota');

-- Update RPC to cancel old records before inserting
