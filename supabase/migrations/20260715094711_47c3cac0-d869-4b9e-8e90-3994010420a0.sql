...
-- 1) VIEW: valor efetivo (usa children[] quando amount pai = 0)
CREATE OR REPLACE VIEW public.saipos_fin_effective AS
SELECT
  s.id,
  s.id_store_fin_transaction,
  s.id_store,
  s.date,
  s.issuance_date,
  s.payment_date,
  ...
  s.conciliated,
  CASE
    WHEN COALESCE(s.amount, 0) <> 0 THEN s.amount
    ELSE COALESCE(
      (SELECT SUM((c->>'amount')::numeric)
         FROM jsonb_array_elements(COALESCE(s.children, '[]'::jsonb)) c),
      0
    )
  END AS amount,
  s.amount AS amount_raw,
  s.desc_store_fin_transaction,
  s.desc_store_category_financial,
  s.desc_store_payment_method,
  s.desc_store_bank_account,
  s.provider_trade_name,
  s.children,
  s.raw,
  s.synced_at,
  s.created_at,
...
GRANT SELECT ON public.saipos_fin_effective TO authenticated;
GRANT SELECT ON public.saipos_fin_effective TO service_role;

-- 2) cashflow_upcoming_bills — agora com janela "ONTEM + HOJE + futuro"
...
AS $$
  ...
  FROM public.saipos_fin_effective
...
-- 3) cashflow_upcoming_bills_daily — usa view efetiva
...
-- 6) cashflow_monthly_consolidated
