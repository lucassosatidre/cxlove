...
BEGIN
  ...
  LOOP
    IF v_total_matched < v_total_liquido_esperado
       AND EXISTS (
    ...
    THEN
      ...
      SET match_status = 'matched',
          match_reason = format('Casa com vendas iFood com expected_date=%s (acumulado R$%s de R$%s)',
    ...
    ELSE
      ...
      SET match_status = 'fora_periodo',
          match_reason = CASE 
            WHEN v_total_matched >= v_total_liquido_esperado THEN
              format('Excedente após cobrir competência (acumulado R$%s)',
                     round(v_total_matched, 2))
            ELSE
              format('Sem vendas iFood com expected_date=%s na competência',
                     to_char(v_dep.deposit_date, 'DD/MM/YYYY'))
          END
      WHERE id = v_dep.id;
    END IF;
