...
CREATE OR REPLACE FUNCTION public.attempt_checkin(
  p_shift_id uuid,
  p_driver_id uuid,
  p_device_ip text DEFAULT NULL,
  p_device_user_agent text DEFAULT NULL,
...
BEGIN
  ...
  -- Try to reuse a cancelled/expired checkin for this driver+shift
  ...
  IF v_checkin_id IS NOT NULL THEN
    UPDATE delivery_checkins SET
      status = v_target_status,
      device_ip = p_device_ip,
      device_user_agent = p_device_user_agent,
      device_info = p_device_info,
      origin = 'entregador',
      confirmed_at = CASE WHEN v_target_status = 'confirmado' THEN v_now ELSE NULL END,
      waitlist_entered_at = CASE WHEN v_target_status = 'fila_espera' THEN v_now ELSE NULL END,
      cancelled_at = NULL,
      cancel_reason = NULL,
