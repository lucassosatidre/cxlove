...
BEGIN
  ...
  -- Iterar por marca com prazos REAIS calculados (P75)
  FOR v_brand, v_prazo IN
    SELECT * FROM (VALUES ('alelo', 3), ('ticket', 6), ('pluxee', 7), ('vr', 7)) AS t(brand, prazo)
  LOOP
    SELECT 
    ...
    LOOP
      -- Janela de venda: deposit_date - prazo até deposit_date - 1
      -- Se essa janela toca o mês de competência, conta como competência
      IF (v_dep.deposit_date - v_prazo) < v_period_end 
         AND v_dep.deposit_date >= v_period_start
         AND (v_dep.deposit_date - 1) >= v_period_start
      THEN
        UPDATE audit_bank_deposits
        SET match_status = 'matched',
            matched_competencia_amount = v_dep.amount,
            matched_adjacente_amount = 0,
            match_reason = format('%s: dep %s, venda esperada em ~%s a %s',
                                  UPPER(v_brand),
                                  to_char(v_dep.deposit_date, 'DD/MM'),
                                  to_char(v_dep.deposit_date - v_prazo, 'DD/MM'),
                                  to_char(v_dep.deposit_date - 1, 'DD/MM'))
        WHERE id = v_dep.id;
      ...
      ELSE
        ...
        SET match_status = 'fora_periodo',
            ...
            match_reason = format('%s adjacente: dep %s = venda de %s a %s (fora competência)',
                                  UPPER(v_brand),
                                  to_char(v_dep.deposit_date, 'DD/MM'),
                                  to_char(v_dep.deposit_date - v_prazo, 'DD/MM'),
                                  to_char(v_dep.deposit_date - 1, 'DD/MM'))
