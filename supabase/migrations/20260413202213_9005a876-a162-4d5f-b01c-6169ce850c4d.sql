...
BEGIN
  -- VALIDAÇÃO DE HORÁRIO: Bloquear check-in fora da janela 15h-17:59
  v_brasilia_hour := EXTRACT(HOUR FROM (v_now AT TIME ZONE 'America/Sao_Paulo'));
  IF v_brasilia_hour < 15 OR v_brasilia_hour >= 18 THEN
    RETURN jsonb_build_object('error', 'Check-in permitido apenas entre 15h e 17:59');
  END IF;
  ...
  -- Validate shift date matches operational date
  ...
  -- Delete any old cancelled/expired records for this driver+shift
