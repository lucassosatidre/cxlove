...
BEGIN
  FOR v_op IN SELECT unnest(ARRAY['alelo','ticket','pluxee','vr']) LOOP
    ...
    -- Líquido esperado: SUM(net_amount) dos lotes com ao menos um item
    -- ligado a venda Maquinona da competência. EXISTS evita duplicação.
    ...
    -- Reset de estado stale (NÃO toca em category)
      ...
      ELSIF (v_acumulado + v_dep.amount) <= v_liquido_esperado THEN
        UPDATE audit_bank_deposits
           SET matched_competencia_amount = v_dep.amount,
               ...
               matched = true,
               match_reason = format(
                 'matched: liquido_esperado=%s acumulado_pos=%s (%.1f%%)',
                 v_liquido_esperado,
                 v_acumulado + v_dep.amount,
                 ((v_acumulado + v_dep.amount) / NULLIF(v_liquido_esperado,0)) * 100
               )
         WHERE id = v_dep.id;
