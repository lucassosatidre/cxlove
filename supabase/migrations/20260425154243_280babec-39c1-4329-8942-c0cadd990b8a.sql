...
BEGIN
  ...
  FOR v_brand IN SELECT unnest(ARRAY['alelo', 'ticket', 'pluxee', 'vr']) LOOP
    ...
    -- Diferença e taxa baseadas no BRUTO de competência (custo real iFood-style)
    ...
    -- Upsert: pode não existir linha (ex. quando run-audit-match filtrou por sold>0 inclusive adjacentes)
