...
function consultarCombo1(menu: MenuData, args: any) {
  const partes: string[] = [...(args.pizza_1 ?? []), ...(args.pizza_2 ?? [])];
  if (partes.length !== 4) return { error: "pizza_1 e pizza_2 precisam de 2 sabores cada (4 partes no total)." };
...
function consultarCombo2(menu: MenuData, args: any) {
  const partes: string[] = args.pizza_gigante ?? [];
  if (partes.length !== 3) return { error: "pizza_gigante precisa de exatamente 3 partes (sabores)." };
...
function consultarMonteDoSeuJeito(menu: MenuData, args: any) {
  ...
  const sabores: string[] = args.sabores ?? [];
  const expected = tamanho === "broto" ? 1 : tamanho === "grande" ? 2 : 3;
  if (sabores.length !== expected) {
    return { error: `tamanho ${tamanho} exige ${expected} sabor(es). Repita o sabor para pizza inteira de 1 sabor.` };
  }
